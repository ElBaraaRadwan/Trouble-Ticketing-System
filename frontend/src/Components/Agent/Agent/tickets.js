...
class Article extends React.Component {
  ...
  get_tickets() {
    axios
      .get("https://trouble-ticketing-system.herokuapp.com/getAllTicket")
      .then((res) => {
  ...
  search(e) {
    if (e.key === "Enter") {
      let key = $(".search").val();
      let r = this.state.tickets.filter(
        (ticket) =>
          (ticket.title.includes(key) ||
            ticket.priorty.includes(key) ||
            ticket.status.includes(key) ||
            ticket.department.includes(key)) &&
          key !== ""
      );
      ...
      <div>
        ...
        <div className="row mt-3 container-fluid">
          <div className="m-auto col-xl-10 col-lg-10">
            ...
            <button
              className="add-blog btn reply-btn"
              onClick={() => {
                this.showAdd();
              }}
            >
              ...
              <Dropdown class="mb-5">
                ...
                {this.state.search_results.length !== 0 && (
                  <Dropdown.Menu>
                    ...
                    <Dropdown.Item
                      href="#/action-1"
                      onClick={(e) => this.sort_by(e, "title")}
                    >
                      title
                    </Dropdown.Item>
                    <Dropdown.Item
                      href="#/action-2"
                      onClick={(e) => this.sort_by(e, "date")}
                    >
                      Recently updated
                    </Dropdown.Item>
                  </Dropdown.Menu>
            ...
            <table className={"p-5 tab " + style["style-table"]}>
              <tr>
                <th className="text-center table-h">title</th>
                <th className="text-center table-h">Priorty</th>
                <th className="text-center table-h">Status</th>
                    ...
                    <td>
                      ...
                      <button
                        ...
                        onClick={() =>
                          axios
                            .delete(
                              "https://trouble-ticketing-system.herokuapp.com/deleteTicket/" +
                                ticket._id
                            )
                            .then((res) => {
