...
class Ticket extends React.Component {
  ...
  load_ticket() {
    axios
      .get(
        "https://trouble-ticketing-system.herokuapp.com/getTicket/" +
  ...
  handleUpdate() {
    ...
    axios
      .patch(
        "https://trouble-ticketing-system.herokuapp.com/updateTicket/" +
          this.props.ticket_id,
        {
          title: title_in,
          description: des_in,
      ...
      .catch((errors) => {
        ...
        if (errors.response.data.errors.description != null) {
          $(".error-bag-des").text(
            errors.response.data.errors.description.message
          );
        }
  ...
  handleReply() {
    ...
    axios
      .patch(
        "https://trouble-ticketing-system.herokuapp.com/solveTicket/" +
          this.props.ticket_id,
        {
          reply: reply_in,
        }
  ...
  handleAssign() {
    ...
    axios
      .patch(
        "https://trouble-ticketing-system.herokuapp.com/assignTicket/" +
          this.props.ticket_id,
        {
          priorty: priorty_in,
          status: status_in,
      ...
      .catch((errors) => {
        ...
        if (errors.response.data.errors.priorty != null) {
          $(".error-bag-priorty").text(
            errors.response.data.errors.priorty.message
          );
        }
        if (errors.response.data.errors.status != null) {
          $(".error-bag-status").text(
            errors.response.data.errors.status.message
          );
        }
      });
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="row mt-3 container-fluid">
          <div className="m-auto col-xl-10 col-lg-10">
            ...
            <div className="task-tabs mt-2 p-5">
              <span
                id="update"
                onClick={() => this.setState({ selected_tab: "update" })}
              >
                Update
              </span>
              <span
                id="reply"
                onClick={() => this.setState({ selected_tab: "reply" })}
              >
                Reply
              </span>
              <span
                id="assign"
                onClick={() => this.setState({ selected_tab: "assign" })}
              >
                Assign
              </span>
            </div>
              ...
              {/* Start our field */}
                                                        <div>
                                                          <p className="details">
                                                            <span className="details-data">created at : {this.state.ticket.createdAt}</span>
                                                            <span className="details-data">priorty : {this.state.ticket.priorty}</span>
                                                            <span className="details-data">status : {this.state.ticket.status}</span>
                                                            <span className="details-data">department : {this.state.ticket.department}</span>
                                                          </p>
                                                          <p>
                                                            <span className="details pl-10 justify-start">
                                                              Reply :<br></br> {this.state.ticket.reply}
                                                            </span>
                                                          </p>
                                                        </div>
            {/* End our field */}
        ...
        */}
              <p align="center">
                <button className="btn reply-btn" onClick={this.handleReply}>
                  reply
                </button>
              </p>
            </div>
            <div id="update-section">
              ...
              <p align="center">
                <button className="btn reply-btn" onClick={this.handleUpdate}>
                  Update
                </button>
              </p>
            </div>
            <div id="assign-section">
              ...
              <p align="center">
                <button className="btn reply-btn" onClick={this.handleAssign}>
                  Assign
                </button>
              </p>
            </div>
